								...
								if(command === "fm"){
									if(!object){
											...
											message.channel.send(err_code);
													...
													message.guild.channels.find("name", "liste-commande-fm").send(hereRole +" Salut le joueur **@"+ message.mentions.members.first().user.username +"** souhaiterait une FM. Prenez contact avec lui dès que vous êtes disponible pour cette commande directement dans le jeu ou en MP Discord ! Voici sa commande: **"+ object +"** . Informations supplémentaires sur la FM: **" + detail + "**");
													} catch (err) {
													console.log(err);
													}
